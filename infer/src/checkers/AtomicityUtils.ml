...
(** A reference to a structure that holds functions whose calls should
    be ignored. *)
let ignoredFunctionCalls : functionsFromFile ref =
  ref {initialised= false; names= SSet.empty}

(** A reference to a structure that holds functions whose analysis should
    be ignored. *)
let ignoredFunctionAnalyses : functionsFromFile ref =
  ref {initialised= false; names= SSet.empty}

(** A reference to a structure that holds functions whose calls should
    be allowed. *)
let allowedFunctionCalls : functionsFromFile ref =
  ref {initialised= false; names= SSet.empty}

(** A reference to a structure that holds functions whose analysis should
    be allowed. *)
let allowedFunctionAnalyses : functionsFromFile ref =
  ref {initialised= false; names= SSet.empty}
    ...
    if Procname.equal f BuiltinDecl.__set_locked_attribute then true
      ...
      Some (actuals : HilExp.t list) ->
        ( match ConcurrencyModels.get_lock_effect f actuals with
          ...
          | GuardLockedIfTrue (_ : HilExp.t) -> true
          | _ -> false
        )
